function initZotpress()
{
	...
	if ( $(".zp-Zotpress-Bib").length > 0 )
	{
		...
		function zp_get_items ( request_start, request_last, $instance, params, update )
		{
			...
			$.ajax(
			{
				...
				data: {
					...
					'api_user_id': $(".ZP_API_USER_ID", $instance).text(),
					'item_type': $(".ZP_DATATYPE", $instance).text(),
					...
					'sort_by': $(".ZP_SORTBY", $instance).text(),
					'order': $(".ZP_ORDER", $instance).text(),
				...
				success: function(data)
				{
					...
					if ( typeof zp_items != 'undefined' && zp_items != null && zp_items != 0 && zp_items.data.length > 0 )
					{
						...
						if ( update === false )
						{
							$("#"+zp_items.instance+" .zp-List").addClass("used_cache");
						}
						else if ( update === true )
						{
							// Remove existing notes temporarily:
							if ( ! $("#"+zp_items.instance+" .zp-List").hasClass("updating")
									&& $("#"+zp_items.instance+" .zp-Citation-Notes").length > 0 )
								$("#"+zp_items.instance+" .zp-Citation-Notes").remove();
							
							if ( ! $("#"+zp_items.instance+" .zp-List").hasClass("updating") )
								$("#"+zp_items.instance+" .zp-List").addClass("updating");
							
							params.zpForceNumsCount = 1;
						}
						
						
						$.each(zp_items.data, function( index, item )
						{
							var tempItem = "";
							
							// Determine item reference
							var $item_ref = $("#"+zp_items.instance+" .zp-List #zp-ID-"+$(".ZP_API_USER_ID", $instance).text()+"-"+item.key);
							
							// Replace or skip duplicates
							if ( $item_ref.length > 0 )
							{
								if ( update === false && ! $("#"+zp_items.instance+" .zp-List").hasClass("used_cache") )
									return false;
								
								//if ( update === true && $("#"+zp_items.instance+" .zp-List").hasClass("used_cache") )
								//	$item_ref.remove();
								//else
								//	return true;
							}
							...
							tempItem += "<div id='zp-ID-"+$(".ZP_API_USER_ID", $instance).text()+"-"+item.key+"'";
							...
							// Image
							if ( $("#"+zp_items.instance+" .ZP_SHOWIMAGE").text().trim().length > 0
							...
							// Force numbers
							if ( $("#"+zp_items.instance+" .ZP_FORCENUM").text().length > 0
									&& $("#"+zp_items.instance+" .ZP_FORCENUM").text() == "1" )
							{
									...
									( item.data.hasOwnProperty('tags') && item.data.tags.length > 0 ) )
							{
								...
								$.each(item.data.tags, function ( tindex, tag )
								{
						...
						if ( update === false ) $("#"+zp_items.instance+" .zp-List").append( tempItems );
						...
						if ( params.zpShowNotes == true && tempNotes.length > 0 )
						{
							...
							$("#"+zp_items.instance).append( tempNotes );
						}
						
						
						// Fix incorrect numbering in existing numbered style
						if ( $("#"+zp_items.instance+" .zp-List .csl-left-margin").length > 0 ) 
						{
							params.zpForceNumsCount = 1; // UNSURE: 0?
							
							$("#"+zp_items.instance+" .zp-List .csl-left-margin").each(function ( index, item )
							{
								var item_content = $(item).text();
								item_content = item_content.replace( item_content.match(/\d+/)[0], params.zpForceNumsCount );
								$(item).text( item_content );
						...
						{
							// Remove loading
							$("#"+zp_items.instance+" .zp-List").removeClass("loading");
							...
							{
								if ( $(".ZP_SORTBY", $instance).text() == "author"
										&& $("#"+zp_items.instance+" .zp-List .csl-left-margin").length == 0 )
								{
									$("#"+zp_items.instance+" .zp-List div.zp-Entry").sort(function(a,b)
									{
					...
					else
					{
						if ( update === true )
						{
							$("#"+$instance.attr("id")+" .zp-List").removeClass("loading");
		...
		$(".zp-Zotpress-Bib").each( function( index, instance )
		{
			var $instance = $(instance);
			var zp_params = new Object();
			
			zp_params.zpItemkey = false; if ( $(".ZP_ITEM_KEY", $instance).text().trim().length > 0 ) zp_params.zpItemkey = $(".ZP_ITEM_KEY", $instance).text();
			zp_params.zpCollectionId = false; if ( $(".ZP_COLLECTION_ID", $instance).text().trim().length > 0 ) zp_params.zpCollectionId = $(".ZP_COLLECTION_ID", $instance).text();
			zp_params.zpTagId = false; if ( $(".ZP_TAG_ID", $instance).text().trim().length > 0 ) zp_params.zpTagId = $(".ZP_TAG_ID", $instance).text();
			
			zp_params.zpAuthor = false; if ( $(".ZP_AUTHOR", $instance).text().trim().length > 0 ) zp_params.zpAuthor = $(".ZP_AUTHOR", $instance).text();
			zp_params.zpYear = false; if ( $(".ZP_YEAR", $instance).text().trim().length > 0 ) zp_params.zpYear = $(".ZP_YEAR", $instance).text();
			zp_params.zpStyle = false; if ( $(".ZP_STYLE", $instance).text().trim().length > 0 ) zp_params.zpStyle = $(".ZP_STYLE", $instance).text();
			zp_params.zpLimit = false; if ( $(".ZP_LIMIT", $instance).text().trim().length > 0 ) zp_params.zpLimit = $(".ZP_LIMIT", $instance).text();
			zp_params.zpTitle = false; if ( $(".ZP_TITLE", $instance).text().trim().length > 0 ) zp_params. zpTitle = $(".ZP_TITLE", $instance).text();
			
			zp_params.zpShowImages = false; if ( $(".ZP_SHOWIMAGE", $instance).text().trim().length > 0 ) zp_params.zpShowImages = $(".ZP_SHOWIMAGE", $instance).text().trim();
			zp_params.zpShowTags = false; if ( $(".ZP_SHOWTAGS", $instance).text().trim().length > 0 ) zp_params.zpShowTags = true;
			zp_params.zpDownloadable = false; if ( $(".ZP_DOWNLOADABLE", $instance).text().trim().length > 0 ) zp_params.zpDownloadable = true;
			zp_params.zpInclusive = false; if ( $(".ZP_INCLUSIVE", $instance).text().trim().length > 0 ) zp_params.zpInclusive = true;
			zp_params.zpShowNotes = false; if ( $(".ZP_NOTES", $instance).text().trim().length > 0 ) zp_params.zpShowNotes = true;
			zp_params.zpShowAbstracts = false; if ( $(".ZP_ABSTRACT", $instance).text().trim().length > 0 ) zp_params.zpShowAbstracts = true;
			zp_params.zpCiteable = false; if ( $(".ZP_CITEABLE", $instance).text().trim().length > 0 ) zp_params.zpCiteable = true;
			zp_params.zpTarget = false; if ( $(".ZP_TARGET", $instance).text().trim().length > 0 ) zp_params.zpTarget = true;
			zp_params.zpURLWrap = false; if ( $(".ZP_URLWRAP", $instance).text().trim().length > 0 ) zp_params.zpURLWrap = $(".ZP_URLWRAP", $instance).text();
			zp_params.zpHighlight = false; if ( $(".ZP_HIGHLIGHT", $instance).text().trim().length > 0 ) zp_params.zpHighlight = $(".ZP_HIGHLIGHT", $instance).text();
			...
			if ( zp_params.zpCollectionId && zp_params.zpCollectionId.indexOf(",") != -1 )
			{
				...
				$.each( tempCollections, function (i, collection)
				{
			...
			else
			{
				...
				else
				{
					if ( zp_params.zpAuthor && zp_params.zpAuthor.indexOf(",") != -1 )
					{
						...
						{
							$.each( tempAuthors, function (i, author)
							{
					...
					else
					{
						if ( zp_params.zpYear && zp_params.zpYear.indexOf(",") != -1 )
						{
							...
							$.each( tempYears, function (i, year)
							{
