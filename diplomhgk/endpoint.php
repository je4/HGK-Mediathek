...
*/
    ...
    if( isset($_GET["done"]) || $totalParts == 1 ) {
      ...
      elseif( preg_match( '/^image\//', $mime ) ) {
        ...
        shell_exec( 'convert '.escapeshellarg( "files/{$uuid}/{$name}" ).' -resize '.escapeshellarg('1920x1080>').' '.escapeshellarg( "web/{$uuid}/{$name}.png" ));
        if( file_exists("thumbs/{$uuid}/{$name}.png")) {
          $result['thumbnailUrl'] = "thumbs/{$uuid}/{$name}.png";
          if( file_exists("thumbs/{$uuid}/{$name}.png")) {
            $result['thumbnailUrl'] = "thumbs/{$uuid}/{$name}.png";
      ...
      elseif( preg_match( '/^application\/pdf/', $mime ) ) {
        ...
        shell_exec( 'convert '.escapeshellarg( "files/{$uuid}/{$name}" ).'[0] -resize '.escapeshellarg('1920x1080>').' '.escapeshellarg( "web/{$uuid}/{$name}.png" ));
        if( file_exists("thumbs/{$uuid}/{$name}.png")) {
          $result['thumbnailUrl'] = "thumbs/{$uuid}/{$name}.png";
          if( file_exists("thumbs/{$uuid}/{$name}.png")) {
            $result['thumbnailUrl'] = "thumbs/{$uuid}/{$name}.png";
      ...
      $sql = "INSERT INTO source_diplomhgk_files( year, idperson, name, filename, thumbname, webname, mimetype, size, tech, uploadtime ) VALUES (
        ...
        , ".(file_exists("web/{$uuid}/{$name}.png") ? $db->qstr( "web/{$uuid}/{$name}.png" ) : "NULL" )."
        , ".$db->qstr( $mime )."
