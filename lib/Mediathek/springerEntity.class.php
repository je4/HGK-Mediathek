...
class springerEntity extends swissbibEntity {
	private $node;
	...
	function loadNode( $id, $doc, $idprefix ) {
        ...
        if( $this->id == null ) {
        	foreach( array( '020', '022' ) as $fld ) {
		        $flds = $this->findField( $fld, ' ', ' ' );
		        $val = null;
		        $type = null;
		        foreach( $flds as $fld ) {
	...
	public function getCoverImg() {
		...
		if( strlen( $isbn ) > 0) {
			return "https://static-content.springer.com/cover/book/{$isbn}.jpg";
		}
		return null;
	}
