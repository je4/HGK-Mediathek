...
*/
    ...
    if( isset($_GET["done"]) || $totalParts == 1 ) {
      ...
      @mkdir( "personpics/{$md5}" );
      list($width, $height) = getimagesize( "files/{$uuid}/{$name}" );
      $theight = 640;
      $twidth = ceil($theight*$width/$height);
      $cmd = "convert ".escapeshellarg( "files/{$uuid}/{$name}" )." -resize {$twidth}x{$theight} ".escapeshellarg( "personpics/{$md5}/".pathinfo($name, PATHINFO_FILENAME)."_small.png" );
      $r = shell_exec( $cmd );
      $cmd = "convert ".escapeshellarg( "personpics/{$md5}/".pathinfo($name, PATHINFO_FILENAME)."_small.png" )." -set option:distort:viewport \"1920x640\"-".(320+$twidth/2)."-0 -virtual-pixel Edge -distort SRT 0 +repage ".escapeshellarg( "personpics/{$md5}/".pathinfo($name, PATHINFO_FILENAME)."_wide.png" );
      $r = shell_exec( $cmd );
      //$result['cmd'] = $cmd;
      //$result['ret'] = $r;
